...
openerp.web_graph.Graph = openerp.web.Widget.extend({
    ...
    start: function() {
        ...
        return this.model.call('fields_get', {
            ...
            self.fields.__percentage = {field:'__percentage', type: 'float', string:_t('Percentage')};
            ...
            self.pivot_options.measures = self.create_field_values(self.pivot_options.measures || [{field:'__count', type: 'integer', string:'Count'},{field:'__percentage', type: 'float', string:'Percentage'}]);
    ...
    make_cell: function (row, col, value, index, raw) {
		...
		if (this.pivot.measures[index]['field'] == '__percentage' && formatted_value.length > 0) {
			formatted_value += '%';
		}
		
		var cell = {value:formatted_value};
